...
RSpec.describe TeamDashboard do
  ...
  it "leaves out a project the reader cannot reach, bond or no bond" do
    hidden = create(:project, ngo: create(:ngo, :active, country: create(:country, high_risk: true)))
    join(hidden)

    aggregate_failures do
      expect(dashboard).to be_on_a_team
      expect(dashboard.boards).to be_empty
    end
  end
  ...
  describe "the money on the board" do
    ...
    it "reads the approved budget and leaves a rejected expense out of the spend" do
      ...
      create(:budget_line, budget: budget, estimated_cents: 400_000)
