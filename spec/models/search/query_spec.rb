...
RSpec.describe Search::Query do
  ...
  describe "the three groups in one box" do
    ...
    it "answers project, base and country for a term that matches all three" do
      base = create(:ngo, :active, country: country, name: "Vale Verde")
      create(:project, ngo: base, title: "Reforma do Vale")
    ...
    it "counts every group together" do
      create(:project, ngo: ngo, title: "Reforma do Vale")
  ...
  describe "accents and case" do
    let!(:accented) { create(:ngo, :active, country: country, name: "Clínica do Sertão") }
    ...
    it "finds accented data from an unaccented term" do
      expect(search("clinica do sertao").groups.fetch(:ngos)).to include(accented)
    end

    it "finds it whatever the case" do
      expect(search("CLÍNICA").groups.fetch(:ngos)).to include(accented)
    end

    it "finds unaccented data from an accented term" do
      plain = create(:ngo, :active, country: country, name: "Casa Norte")

      expect(search("Cása").groups.fetch(:ngos)).to include(plain)
    end
  end

  describe "what a reader cannot reach" do
    let(:hidden_country) { create(:country, high_risk: true) }
    let!(:hidden) { create(:ngo, :active, country: hidden_country, name: "Base Escondida") }
    ...
    it "answers it to a reader who reaches it" do
      expect(search("escondida").groups.fetch(:ngos)).to include(hidden)
    end
  ...
  describe "with no term at all" do
    ...
    it "answers everything the reader reaches" do
      expect(search(nil).groups.transform_values(&:count)).to eq(projects: 1, ngos: 1, countries: 1)
    end
