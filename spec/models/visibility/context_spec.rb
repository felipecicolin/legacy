...
RSpec.describe Visibility::Context do
  ...
  # Três perguntas, três nomes, uma regra hoje. Os exemplos são separados
  ...
  it "refuses the attachment of a record above the clearance" do
    record = SensitiveTestRecord.new(sensitivity_level: :confidential)

    expect(described_class.anonymous).not_to be_can_see_attachment(record)
  end

  it "allows the attachment of a record within the clearance" do
    record = SensitiveTestRecord.new(sensitivity_level: :public)

    expect(described_class.anonymous).to be_can_see_attachment(record)
  end
end
