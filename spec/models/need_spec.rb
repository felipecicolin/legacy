...
RSpec.describe Need do
  ...
  describe "where a need hangs from" do
    ...
    it "accepts a need on a base with no project at all" do
      expect(build(:need, mission_base: mission_base, project: nil)).to be_valid
    end

    it "accepts a need on a project of its own base" do
      expect(build(:need, :for_project, mission_base: mission_base)).to be_valid
    end
    ...
    it "refuses a project that belongs to another base" do
      ...
      expect(build(:need, mission_base: mission_base, project: elsewhere)).not_to be_valid
    end
  end

  describe "the skill that only one kind carries" do
    it "refuses a skill need with no skill" do
      expect(build(:need, mission_base: mission_base, need_kind: :skill, skill: nil)).not_to be_valid
    end
    ...
    it "refuses a material need that carries a skill" do
      expect(build(:need, mission_base: mission_base, need_kind: :material, skill: create(:skill))).not_to be_valid
    end

    it "accepts a skill need with a skill" do
      expect(build(:need, :skilled, mission_base: mission_base)).to be_valid
    end
  end
  ...
  describe "the quantity and the status derived from it" do
    it "walks from open to partially fulfilled to fulfilled on its own" do
      need = create(:need, mission_base: mission_base, quantity: 10)
    ...
    it "leaves a cancelled need cancelled" do
      need = create(:need, mission_base: mission_base, quantity: 10, need_status: :cancelled)
    ...
    it "refuses more fulfilled than asked for" do
      expect(build(:need, mission_base: mission_base, quantity: 2, fulfilled_quantity: 3)).not_to be_valid
    end

    it "refuses a quantity of zero" do
      expect(build(:need, mission_base: mission_base, quantity: 0)).not_to be_valid
    end
    ...
    it "refuses more fulfilled than asked for in the database too" do
      need = create(:need, mission_base: mission_base, quantity: 2)
  ...
  describe "sensitivity" do
    let(:risky) { create(:mission_base, country: create(:country, high_risk: true)) }

    it "inherits from the base when there is no project" do
      expect(create(:need, mission_base: risky)).to be_confidential
    end

    it "inherits from the project when there is one" do
      project = create(:project, mission_base: risky)

      expect(create(:need, mission_base: risky, project: project)).to be_confidential
    end

    it "keeps the closed default when the source is more open" do
      expect(create(:need, mission_base: mission_base).sensitivity_level).to eq("restricted")
    end

    it "survives having no base yet, so the presence validation is the one that speaks" do
      expect(build(:need, mission_base: nil)).not_to be_valid
    end
  end
  ...
  describe ".matching" do
    ...
    it "leaves out a need that was already fulfilled" do
      create(:need, mission_base: mission_base, need_kind: :skill, skill: skill, quantity: 1, fulfilled_quantity: 1)
  ...
  describe ".by_priority" do
    it "puts the most urgent first, regardless of when it was created" do
      low = create(:need, mission_base: mission_base, urgency: :low)
      critical = create(:need, mission_base: mission_base, urgency: :critical)

      expect(described_class.by_priority).to eq([critical, low])
    end

    it "breaks a tie by the closest deadline" do
      later = create(:need, mission_base: mission_base, needed_by: 1.month.from_now.to_date)
      sooner = create(:need, mission_base: mission_base, needed_by: 1.week.from_now.to_date)
  ...
  describe "labels and presentation" do
    let(:need) { build(:need, mission_base: mission_base, need_kind: :team, urgency: :critical) }
    ...
    it "interpolates as its title" do
      expect(build(:need, mission_base: mission_base, title: "Telhas").to_s).to eq("Telhas")
    end
  end

  it "refuses a negative estimate and a currency that is not a three letter code" do
    aggregate_failures do
      expect(build(:need, mission_base: mission_base, estimated_value_cents: -1)).not_to be_valid
      expect(build(:need, mission_base: mission_base, currency: "REAIS")).not_to be_valid
    end
  end
end
