...
class CreateProjectParticipations < ActiveRecord::Migration[8.1]
  def change
    ...
    # `role` entra no índice único de propósito: a mesma pessoa pode ser
    # `technical_lead` E `local_host` na mesma obra, e deixá-lo de fora
    # proibiria um caso real.
    add_index :project_participations, %i[project_id profile_id role], unique: true
    add_index :project_participations, %i[profile_id status]
